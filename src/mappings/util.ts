...
const utcTime = (timestamp: Date, keepLocalTime: boolean = true) => Mom(timestamp).utc(keepLocalTime)
        ...
        switch (policy) {
            case SnapshotPolicy.Hourly:
                ...
                if (hitHourTime(timestamp, cache.timestamp, 1)) {
                    logger.debug(`hourly snapshot policy: ${blockHeight}-${timestamp}\n last cache: %o`, cache)
...
export async function blockHandleWrapper(block: SubstrateBlock, handler: (block: SubstrateBlock) => Promise<void>) {
